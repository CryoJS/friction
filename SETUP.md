...
| `CRAWL_FETCH_MAX` | | `20` | Pages a scan's crawl surveys over Browserbase's **Fetch API** before it opens a browser (0-200). Fetch costs about $1 per 1000 pages and takes no session, so the survey is far wider than the crawl used to be. `0` turns the pre-pass off and the crawl is the session-based one it always was |
| `CRAWL_SESSION_MAX` | | `3` | Most surveyed pages that earn a browser (0-6). Fetch does not run JavaScript, so client-rendered pages and the most interactive flows get one session, opened once and reused for all of them |
